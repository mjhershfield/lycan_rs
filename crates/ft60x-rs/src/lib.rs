...
impl Device {
    ...
    /// Open a device using the given serial number.
    pub fn open_with_index(index: u32) -> Result<Device> {
        unsafe {
            let mut handle: types::FT_HANDLE = std::ptr::null_mut();
            lib::FT_Create(
                index as *mut c_void,
                constants::FT_OPEN_BY_INDEX,
                &mut handle as *mut types::FT_HANDLE,
            )?;
            Ok(Self::from_handle(handle))
        }
    ...
    pub fn write(&self, pipe: Pipe, buf: &[u8], timeout: Duration) -> Result<usize> {
        if !pipe.is_write_pipe() {
            Err(D3xxError::InvalidParameter)?;
        }

        let mut bytes_transferred: c_ulong = 0;
        unsafe {
            match lib::FT_WritePipe(
                self.handle,
                pipe as c_uchar,
                buf as *const _ as *const c_uchar,
                buf.len() as c_ulong,
    ...
    /// until the transfer is complete, or the timeout is reached.
    pub fn read(&self, pipe: Pipe, buf: &mut [u8], timeout: Duration) -> Result<usize> {
